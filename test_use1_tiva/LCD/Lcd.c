 ...
 * */
void LCD_VoidMoveCursorXY(uint08_t row , uint08_t column){
        uint08_t address;
...
void LCD_VoidDisplayStringRowColumn(uint08_t row,uint08_t col,const char *Str){
    ...
    LCD_VoidMoveCursorXY(row,col);   /* go to to the required LCD position */
...
static void Write(uint08_t data)
{
    ...
    /* Out the first 4 bits of the required command to the data bus D0 --> D3 */
    d = GPIO_readPort(LCD_PORT_ID);
    d &= 0xF0;                // Clear the lower 4 bits
    d |= (data >> 4);         // Shift the higher 4 bits of 'data' to the lower 4 bits of 'd'
    GPIO_SetPortState(LCD_PORT_ID, d);
    EnPulse();

    /* Repeat for the second nibble */
    d = GPIO_readPort(LCD_PORT_ID);
    d &= 0xF0;                // Clear the lower 4 bits again
    d |= (data & 0x0F);       // Mask the lower 4 bits of 'data' and directly assign to 'd'
    GPIO_SetPortState(LCD_PORT_ID, d);
    EnPulse();
